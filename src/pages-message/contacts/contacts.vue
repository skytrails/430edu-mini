  ...
  if (keyword.value) {
    ...
    originData.value.forEach((item) => {
      let data = item.data.filter(
        (inItem) => inItem.realname.indexOf(keyword.value) != -1,
      );
...
const handleResult = (arr) => {
  let newArr = [];
  arr.forEach((item) => {
    let { id, realname, avatar, username, phone, email, post, orgCodeTxt } =
      item;
    //聊天通讯录把自己过滤掉
...
const fillAZIndexes = (data) => {
  // 生成A-Z的字母数组
  const allIndexes = Array.from({ length: 26 }, (_, i) =>
...
.wraper {
  // height: 100%;
  height: calc(
    100vh - var(--window-top) - constant(safe-area-inset-bottom) - 140px
  );
